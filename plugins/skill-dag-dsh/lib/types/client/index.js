...
function arrow(id, color) {
    return React.createElement('marker', { id, viewBox: '0 0 10 10', refX: '9', refY: '5', markerWidth: '7', markerHeight: '7', orient: 'auto' }, React.createElement('path', { d: 'M0,0 L10,5 L0,10 z', fill: color }));
}
...
function DAGView(props) {
    ...
    if (!dag)
        return React.createElement('div', { style: { color: '#94a3b8', fontSize: 12 } }, 'No DAG (routed to ReAct fallback, or not compiled yet).');
    ...
    const nodeEls = nodes.map(function (n) {
        ...
        const details = n.id
            ...
            + '\neff: ' + ((n.effect || []).join(', ') || '—');
        return React.createElement('g', { key: n.id }, React.createElement('title', null, details), React.createElement('rect', { x, y, width: nodeW, height: nodeH, rx: 8, fill: '#0f172a', stroke: fill, strokeWidth: 2 }), React.createElement('text', { x: x + nodeW / 2, y: y + nodeH / 2 + 4, fill: '#f1f5f9', fontSize: 12, fontWeight: 600, textAnchor: 'middle' }, n.name || n.id));
    });
    const defs = React.createElement('defs', null, arrow('ah-state', '#3b82f6'), arrow('ah-data', '#a855f7'), arrow('ah-order', '#94a3b8'));
    return React.createElement('svg', { ref: svgRef, viewBox: '0 0 ' + maxX + ' ' + maxY, style: { background: '#020617', borderRadius: 8, display: 'block', width: '100%', height: 'auto', cursor: 'grab', touchAction: 'none', userSelect: 'none' }, onMouseDown, onMouseMove, onMouseUp, onMouseLeave: onMouseUp }, defs, React.createElement('g', { transform: 'translate(' + pan.x + ' ' + pan.y + ')' }, edgeEls, nodeEls));
}
/** Tool-card view: render the DAG returned by grasp_compile_task as SVG inline in the chat. */
function GraspDagToolView(props) {
    ...
    if (!dag) {
        return React.createElement('div', { style: { font: '12px/1.4 system-ui, sans-serif', color: '#e2e8f0', padding: 8, background: '#0f172a', border: '1px solid #334155', borderRadius: 8 } }, failReason
            ? React.createElement('div', { style: { color: '#f87171' } }, failReason)
            : React.createElement('div', { style: { color: '#94a3b8' } }, 'No DAG compiled.'), React.createElement('pre', { style: { whiteSpace: 'pre-wrap', margin: '6px 0 0', color: '#94a3b8', fontSize: 11 } }, text));
    }
    ...
    return React.createElement('div', { style: { font: '12px/1.4 system-ui, sans-serif', color: '#e2e8f0', padding: 8, background: '#0f172a', border: '1px solid #334155', borderRadius: 8 } }, React.createElement('div', { style: { display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 6 } }, React.createElement('b', null, 'GraSP DAG'), React.createElement('span', { style: { color: '#94a3b8' } }, dag.filtered || ''), React.createElement('span', { style: { color: '#7dd3fc' } }, routing + (conf ? ' c_ret=' + conf : ''))), plan ? React.createElement('div', { style: { marginBottom: 6, color: '#eab308' } }, 'plan: ' + plan) : null, React.createElement(DAGView, { dag: dag }));
}
